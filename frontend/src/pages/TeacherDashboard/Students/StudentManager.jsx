    ...
    return (
        <div className="animate-fade-in">
            <div style={{ marginBottom: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                <div>
                    <h1 style={{ fontSize: '2.5rem', fontWeight: 800, marginBottom: '8px', color: '#1a1a2e' }}>My Students</h1>
                    <p style={{ color: '#64748b', fontSize: '1.1rem' }}>Track progress and manage enrolled scholars across your courses.</p>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px', marginBottom: '40px' }}>
                <GlassCard style={{ padding: '24px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
                        ...
                        <div>
                            <p style={{ color: '#64748b', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}>Total Enrollments</p>
                ...
                <GlassCard style={{ padding: '24px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
                        ...
                        <div>
                            <p style={{ color: '#64748b', fontSize: '0.9rem', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}>Your Requests</p>
            ...
            <GlassCard heavy style={{ padding: '0', overflow: 'hidden' }}>
                {loading ? (
                    <div style={{ padding: '60px', textAlign: 'center' }}>
                        ...
                        <p style={{ color: '#64748b' }}>Loading student metrics...</p>
                    </div>
                ) : students.length === 0 ? (
                    <div style={{ padding: '60px', textAlign: 'center' }}>
                        <GraduationCap size={48} style={{ opacity: 0.2, margin: '0 auto 16px' }} />
                        <h3 style={{ fontSize: '1.2rem', fontWeight: 600, color: '#64748b' }}>No students enrolled yet</h3>
                    </div>
                ) : (
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                            <thead>
                                <tr style={{ background: 'rgba(0, 0, 0, 0.03)', borderBottom: '1px solid rgba(0, 0, 0, 0.06)' }}>
                                    <th style={{ padding: '20px', fontSize: '0.8rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase' }}>Student</th>
                                    <th style={{ padding: '20px', fontSize: '0.8rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase' }}>Course</th>
                                    <th style={{ padding: '20px', fontSize: '0.8rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase' }}>Progress Track</th>
                                    <th style={{ padding: '20px', fontSize: '0.8rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase' }}>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {filteredStudents.map((enrollment) => {
                                    ...
                                    return (
                                        <tr key={enrollment.enrollment_id} style={{ borderBottom: '1px solid rgba(0, 0, 0, 0.06)' }}>
                                            <td style={{ padding: '20px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                                    ...
                                                    <div>
                                                        <h4 style={{ fontWeight: 600, color: '#1a1a2e', marginBottom: '2px' }}>{enrollment.student_name}</h4>
                                                        <p style={{ fontSize: '0.8rem', color: '#64748b' }}>{enrollment.student_email}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td style={{ padding: '20px' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                    <div style={{ flex: 1, minWidth: '100px', height: '6px', background: 'rgba(0, 0, 0, 0.06)', borderRadius: '3px', overflow: 'hidden' }}>
            ...
            {showModal && selectedStudent && (
                 <div style={{
                    ...
                    <GlassCard heavy style={{ maxWidth: '500px', width: '100%', position: 'relative' }}>
                        <button onClick={() => setShowModal(false)} style={{ position: 'absolute', top: '24px', right: '24px', background: 'transparent', border: 'none', color: '#64748b', cursor: 'pointer' }}>
                            <XCircle size={24} />
                        </button>
                        ...
                        <div style={{ marginBottom: '24px' }}>
                            ...
                            <p style={{ color: '#475569', fontSize: '0.9rem' }}>
                                For student: <strong style={{ color: '#1a1a2e' }}>{selectedStudent.student_name}</strong>
                            </p>
                        </div>

                        <form onSubmit={submitSuspensionRequest} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            <div>
                                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#64748b', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Reason for Suspension</label>
                                <textarea 
                                    className="glass-input"
                                    required
                            ...
                            <div>
                                <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 600, color: '#64748b', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Attach Proof (Image/PDF)</label>
                                <div style={{ position: 'relative', border: '1px dashed rgba(255,255,255,0.2)', padding: '20px', borderRadius: '16px', background: 'rgba(0, 0, 0, 0.02)', textAlign: 'center' }}>
                                    <input 
                                        type="file" 
                            ...
                            <div style={{ display: 'flex', gap: '12px', marginTop: '12px' }}>
                                ...
                                <GlassButton type="submit" disabled={isSubmitting || !proofFile} style={{ flex: 2, justifyContent: 'center', background: '#ff453a', border: 'none', color: '#1a1a2e', fontWeight: 700 }}>
