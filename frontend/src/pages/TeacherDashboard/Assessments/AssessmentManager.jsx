    ...
    return (
        <div className="animate-fade-in" style={{ padding: '0 20px' }}>
            <div style={{ marginBottom: '56px' }}>
                <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 3.5rem)', fontWeight: 800, marginBottom: '16px', color: '#1a1a2e', letterSpacing: '-0.02em' }}>Assessments Hub</h1>
                    ...
                    <div key={course.id} style={{ position: 'relative' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '32px', padding: '0 4px', flexWrap: 'wrap', gap: '24px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                                <div style={{ width: '56px', height: '56px', borderRadius: '16px', background: 'linear-gradient(135deg, rgba(10, 132, 255, 0.2), rgba(191, 90, 242, 0.2))', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(0, 0, 0, 0.08)', boxShadow: '0 8px 16px rgba(0,0,0,0.2)' }}>
                                    <BookOpen size={28} color="white" />
                                </div>
                                <div>
                                    <h2 style={{ fontSize: '1.8rem', fontWeight: 700, color: '#1a1a2e', letterSpacing: '-0.01em' }}>{course.title}</h2>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'var(--accent-blue)' }}></div>
                                        <p style={{ fontSize: '0.9rem', color: '#64748b', fontWeight: 500 }}>Active Management</p>
                                    </div>
                                </div>
                            </div>
                            <div style={{ display: 'flex', gap: '12px' }}>
                                <Link to={`/teacher/assessments/quiz/create?course=${course.id}`} style={{ textDecoration: 'none' }}>
                                    <GlassButton style={{ fontSize: '0.9rem', gap: '10px', padding: '12px 24px', background: 'rgba(0, 0, 0, 0.03)', fontWeight: 600 }}>
                                        <Plus size={18} /> Quiz
                                    </GlassButton>
                                </Link>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 420px), 1fr))', gap: '24px', alignItems: 'stretch' }}>
                            ...
                            {(course.modules?.every(m => !m.quizzes?.length && !m.assignments?.length)) && (
                                <div style={{ gridColumn: '1 / -1', padding: '80px 40px', textAlign: 'center', background: 'rgba(0, 0, 0, 0.02)', borderRadius: '28px', border: '1px dashed rgba(255,255,255,0.1)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
...
const AssessmentCard = ({ type, item, moduleTitle, courseId, onDelete }) => {
    ...
    return (
        <GlassCard style={{ padding: '32px', display: 'flex', flexDirection: 'column', height: '100%', border: '1px solid rgba(0, 0, 0, 0.06)', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1 }}>
                ...
                <h3 style={{ fontSize: '1.4rem', fontWeight: 700, marginBottom: '8px', color: '#1a1a2e', letterSpacing: '-0.02em', lineHeight: '1.3' }}>{item.title}</h3>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(0, 0, 0, 0.03)', padding: '6px 12px', borderRadius: '8px', width: 'fit-content', marginBottom: '28px', border: '1px solid rgba(0, 0, 0, 0.04)' }}>
                    <BookOpen size={14} color="var(--accent-blue)" /> 
                    <span style={{ fontSize: '0.85rem', color: '#64748b', fontWeight: 500 }}>{moduleTitle}</span>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', padding: '20px', background: 'rgba(0,0,0,0.2)', borderRadius: '20px', border: '1px solid rgba(0, 0, 0, 0.04)', boxShadow: 'inset 0 2px 20px rgba(0,0,0,0.2)' }}>
                    {type === 'quiz' ? (
                        <>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                    <Clock size={14} color="var(--accent-purple)" />
                                    <span style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 600 }}>Duration</span>
                                </div>
                                <span style={{ fontSize: '1rem', color: '#1a1a2e', fontWeight: 600 }}>{item.time_limit_mins} mins</span>
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                    <RotateCcw size={14} color="var(--accent-purple)" />
                                    <span style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 600 }}>Attempts</span>
                                </div>
                                <span style={{ fontSize: '1rem', color: '#1a1a2e', fontWeight: 600 }}>{item.max_retakes} Max</span>
                            </div>
                        </>
                    ) : (
                        <div style={{ gridColumn: '1 / -1', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                ...
                                <span style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 600 }}>Submission Deadline</span>
                            </div>
                            <span style={{ fontSize: '1rem', color: '#1a1a2e', fontWeight: 600 }}>
