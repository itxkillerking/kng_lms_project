...
const Profile = () => {
    ...
    const renderTextBlocks = (text) => {
        if (!text) return <p style={{ fontSize: '1.05rem', color: 'rgba(255,255,255,0.4)', fontStyle: 'italic' }}>Not provided.</p>;
        
        const paragraphs = text.split(/\n+/).map(p => p.trim()).filter(p => p.length > 0);
        if (paragraphs.length === 0) return <p style={{ fontSize: '1.05rem', color: 'rgba(255,255,255,0.4)', fontStyle: 'italic' }}>Not provided.</p>;
    ...
    if (!user) return <div style={{ display: 'flex', justifyContent: 'center', padding: '100px', color: 'white' }}>Loading Profile...</div>;
    ...
    return (
        <div className="animate-fade-in" style={{ padding: isExtraSmall ? '40px 16px' : isMobile ? '40px 24px' : '40px 40px', maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ marginBottom: '40px', textAlign: isTablet ? 'center' : 'left' }}>
                <h1 style={{ fontSize: isExtraSmall ? '2rem' : '2.5rem', fontWeight: 800, marginBottom: '8px', color: 'white', letterSpacing: '-0.03em' }}>Your Profile</h1>
            ...
            <div style={{ 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', position: isTablet ? 'static' : 'sticky', top: '40px' }}>
                    <GlassCard heavy style={{ padding: isExtraSmall ? '24px' : '32px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', textAlign: 'left', marginTop: '16px', padding: '20px', background: 'rgba(255,255,255,0.02)', borderRadius: '20px', width: '100%', border: '1px solid rgba(255,255,255,0.03)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '0.9rem', color: 'rgba(255,255,255,0.6)', fontWeight: 600 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    <GlassCard heavy style={{ padding: isExtraSmall ? '24px' : isMobile ? '32px' : '48px', borderRadius: '32px' }}>
                        <div style={{ display: 'flex', flexDirection: isExtraSmall ? 'column' : 'row', justifyContent: 'space-between', alignItems: isExtraSmall ? 'flex-start' : 'center', marginBottom: '36px', gap: '16px', paddingBottom: '24px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '32px' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <label style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase' }}>First Name</label>
                                {isEditing ? (
                                    <input className="glass-input" value={formData.first_name} onChange={e => setFormData({...formData, first_name: e.target.value})} style={{ background: 'rgba(255,255,255,0.03)' }} />
                                ) : (
                                    <p style={{ fontSize: '1.15rem', fontWeight: 600, padding: '4px 0' }}>{user.first_name || 'Not provided'}</p>
                                )}
                            </div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <label style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Last Name</label>
                                {isEditing ? (
                                    <input className="glass-input" value={formData.last_name} onChange={e => setFormData({...formData, last_name: e.target.value})} style={{ background: 'rgba(255,255,255,0.03)' }} />
                                ) : (
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '32px', marginTop: '32px' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <label style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Email Address</label>
                                {isEditing ? (
                                    <input className="glass-input" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} style={{ background: 'rgba(255,255,255,0.03)' }} />
                                ) : (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                <label style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Phone Number</label>
                                {isEditing ? (
                                    <input className="glass-input" placeholder="+1 (555) 000-0000" value={formData.phone_number} onChange={e => setFormData({...formData, phone_number: e.target.value})} style={{ background: 'rgba(255,255,255,0.03)' }} />
                                ) : (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '1.1rem', fontWeight: 500, padding: '4px 0' }}>
                                        <div style={{ background: 'rgba(10, 132, 255, 0.1)', padding: '6px', borderRadius: '8px' }}>
                        ...
                        {user.role === 'instructor' && (
                            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '32px', marginTop: '40px', paddingTop: '40px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                                    <label style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Professional Title</label>
                                    {isEditing ? (
                                        <input className="glass-input" value={formData.instructor_title} onChange={e => setFormData({...formData, instructor_title: e.target.value})} placeholder="e.g. Senior Software Architect" style={{ background: 'rgba(255,255,255,0.03)' }} />
                                    ) : (
                                        <p style={{ fontSize: '1.2rem', fontWeight: 700, padding: '4px 0', color: 'white' }}>{user.instructor_title || 'Expert Instructor'}</p>
                                    )}
                                </div>
                            </div>
                        )}

                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '40px', paddingTop: '40px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                            <label style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Bio / Profile Summary</label>
                            {isEditing ? (
                                <textarea className="glass-input" style={{ minHeight: '140px', resize: 'vertical', background: 'rgba(255,255,255,0.03)', padding: '20px', fontSize: '1rem', lineHeight: 1.6 }} value={formData.bio} onChange={e => setFormData({...formData, bio: e.target.value})} placeholder="Tell us about yourself. Use empty lines to separate paragraphs." />
                            ) : (
                        ...
                        {user.role === 'instructor' && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '40px', paddingTop: '40px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                                <label style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.3)', fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Professional Experience Timeline</label>
                                {isEditing ? (
                                    <textarea className="glass-input" style={{ minHeight: '220px', resize: 'vertical', background: 'rgba(255,255,255,0.03)', padding: '20px', fontSize: '1rem', lineHeight: 1.6 }} value={formData.experience} onChange={e => setFormData({...formData, experience: e.target.value})} placeholder="Describe your career journey and achievements.&#10;&#10;Use empty lines to separate distinct experiences into individual milestone cards." />
                                ) : (
            ...
            {showCropModal && (
                <div style={{
                    ...
                    <GlassCard heavy style={{ width: '100%', maxWidth: '450px', padding: '0', overflow: 'hidden', borderRadius: '32px', display: 'flex', flexDirection: 'column', border: '1px solid rgba(255,255,255,0.08)' }}>
                        <div style={{ padding: '24px 32px', borderBottom: '1px solid rgba(255,255,255,0.03)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(255,255,255,0.02)' }}>
                            <h3 style={{ fontSize: '1.2rem', fontWeight: 800 }}>Adjust Profile Picture</h3>
                            <button onClick={() => setShowCropModal(false)} style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '50%', width: '36px', height: '36px', border: 'none', color: 'rgba(255,255,255,0.8)', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                <X size={18} />
                            </button>
                        </div>
                        
                        <div style={{ position: 'relative', width: '100%', height: '400px', background: '#05050A' }}>
                            <Cropper
                                image={imageSrc}
                                crop={crop}
                                zoom={zoom}
                        ...
                        <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '32px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                                ...
                                <input
                                    ...
                                    style={{ flex: 1, accentColor: '#0A84FF', height: '4px', background: 'rgba(255,255,255,0.1)', borderRadius: '2px', appearance: 'none' }}
                                />
