...
const CourseDetailPage = () => {
    ...
    if (!course) return <div style={{ padding: '80px', textAlign: 'center', color: 'white' }}>Course not found.</div>;
                        ...
                        <div style={{ display: isMobile ? 'contents' : 'block' }}>
                            <div>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginBottom: '32px', flexWrap: 'wrap' }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'rgba(255,255,255,0.4)', fontSize: '0.95rem', fontWeight: 500 }}>
                                ...
                                <div style={{ display: 'flex', gap: isSmallMobile ? '20px' : '32px', flexWrap: 'wrap' }}>
                                    {sections.map((s, i) => (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                            ...
                                            <div>
                                                <p style={{ fontSize: '0.75rem', fontWeight: 800, textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', letterSpacing: '1px' }}>{s.label}</p>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                {course.modules?.map((m, i) => (
                                    <GlassCard key={m.id} style={{ padding: '24px', borderRadius: '20px' }}>
                                        ...
                                        {m.lessons?.map((l) => (
                                            <div key={l.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0 12px 40px', borderTop: '1px solid rgba(255,255,255,0.03)' }}>
                                                <PlayCircle size={14} color="rgba(255,255,255,0.3)" />
                                                <span style={{ fontSize: '0.95rem', color: 'rgba(255,255,255,0.6)' }}>{l.title}</span>
                                            </div>
                                        ))}
                                    </GlassCard>
                                ))}
...
const EnrollmentCard = ({ course, isMobile, isSmallMobile, handleEnroll, enrolling, isEnrolled }) => (
    <GlassCard heavy style={{ position: isMobile ? 'static' : 'sticky', top: '40px', padding: isSmallMobile ? '24px' : '32px', borderRadius: '32px', width: '100%', border: '1px solid rgba(255,255,255,0.08)' }}>
        <div style={{ width: '100%', aspectRatio: '16/9', background: 'rgba(255,255,255,0.03)', borderRadius: '20px', marginBottom: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
            {course.thumbnail ? (
        ...
        <div style={{ marginBottom: '24px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px' }}>
                ...
                {course.price > 0 && <span style={{ color: 'rgba(255,255,255,0.4)', textDecoration: 'line-through', fontSize: '1.2rem' }}>$199.99</span>}
            </div>
